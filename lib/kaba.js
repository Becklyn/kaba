...
class Kaba
{
    ...
    async buildScss ()
    {
        const compiler = new KabaScss({
            ...
            isWatch: this.config.isWatch(),
            lint: this.config.isLint(),
            analyze: false,
            cwd: this.cwd,
            browserlist: this.browserList,
        });

        Object.keys(this.scssEntries).forEach(
            name =>
            {
                const src = this.scssEntries[name];
                const outputPath = `${this.outputPath}/${name}.css`;
