    ...
    async run ()
    {
        ...
        return new Promise(
            (resolve) =>
            {
                ...
                if (null != this.buildConfig.js.tsConfigFile)
                {
                    let truncatedPath = this.buildConfig.js.tsConfigFile.replace(process.cwd(), ".");

                    this.logger.log(`Using custom TypeScript config: ${kleur.yellow(truncatedPath)}`);
                }
