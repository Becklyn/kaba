    ...
    async run ()
    {
        ...
        return new Promise(
            (resolve) =>
            {
                ...
                {
                    this.logger.log(`Using custom TypeScript config: ${kleur.yellow(path.relative(this.buildConfig.cwd, this.buildConfig.js.customTypeScriptConfig))}`);
                }
                ...
                else
                {
                    compiler.run(
                        (error, stats) => {
                            this.logger.logBuildSuccess("Build finished", process.hrtime(start));
