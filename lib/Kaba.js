...
class Kaba
{
    ...
    addSassEntries (mapping)
    {
        ...
        {
            console.log(`${kleur.red("ERROR")} It seems that ${kleur.yellow("kaba-scss")} is not installed. Install it with ${kleur.blue("npm install -D kaba-scss")}`);
            process.exit(1);
        }
    ...
    getBuildConfig (cliConfig)
    {
        ...
        if (null != this.customTypeScriptConfig)
        {
            buildConfig.js.tsConfigFile = this.customTypeScriptConfig;
        }
    ...
    buildWebpackConfig (cliConfig)
    {
        ...
        if (this.typescript.enabled)
        {
            ...
            let tsConfig = path.join(this.libRoot, "tsconfig.json");
            let customTsConfig = path.join(this.cwd, "tsconfig.json");

            if (fs.existsSync(customTsConfig))
            {
                tsConfig = customTsConfig;
                this.customTypeScriptConfig = customTsConfig;
            }
